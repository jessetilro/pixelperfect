...
/**
 * Class that displays relevant information in the heads-up display during the
 * gameplay when the debug mode has been activated.
 * 
...
public class DebugHeadsUpDisplay {
	...
	/**
	 * Sets-up text display for the in-game debug HUD, along with all of its
	 * elements.
	 */
	...
	private void setUpBitmapText() {
		...
		captainLog.setLocalTranslation(screenWidth, screenHeight - Constants.DEBUG_LOG_HEIGHT_OFFSET, 0);

		// Init for the HUD text, including font loading and text setting.
		shipHealth = new BitmapText(hudFont, true);
		shipHealth.setLocalTranslation(screenWidth, screenHeight - Constants.DEBUG_HEALTH_HEIGHT_OFFSET, 0);

		teamScore = new BitmapText(hudFont, true);
		teamScore.setLocalTranslation(screenWidth, screenHeight - Constants.DEBUG_SCORE_HEIGHT_OFFSET, 0);
		...
		playersConnected.setLocalTranslation(screenWidth, screenHeight - Constants.DEBUG_CONNECTED_HEIGHT_OFFSET, 0);

		localAddress = new BitmapText(hudFont, true);
		localAddress.setLocalTranslation(screenWidth, screenHeight - Constants.DEBUG_IP_HEIGHT_OFFSET, 0);
	}

	/**
	 * Method responsible for the updating of the debug hud text and displaying
	 * the events, along with their remaining time and health, and other HUD
	 * elements.
	 */
	...
	/**
	 * Clears the debug display after de-activation of the debug mode. The
	 * strings are only cleared instead of detached due to the ability to
	 * re-start the debug mode, which causes issues if re-initialized.
	 */
