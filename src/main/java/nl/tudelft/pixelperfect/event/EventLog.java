...
/**
 * The captain's log of events, which should be subscribed to the event
 * schedulers in the game.
 * 
...
public class EventLog implements EventListener {
	 ...
	 * 
	 * @param event
	 *            The event that the scheduler introduces, to be listed in the
	 *            log.
	 */
	...
	/**
	 * Try to complete an Event in the log that matches a given type and
	 * parameters.
	 * 
	 * @param type
	...
	public synchronized void complete(EventTypes type, Collection<EventParameter> parameters, Game game) {
		...
		if (candidates.size() > 0) {
			...
			System.out.println("Wrong task performed: there is no active Event of type " + type.toString());
		}
	...
	/**
	 * Evaluate the log to check whether events have expired since the last
	 * update.
	 */
